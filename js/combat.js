...
import { sfx } from './audio.js'; // Звуки

// ... вспомогательные функции ...
function getFireDamageTotal(diceCount, diceSides) {
    let calcRoll = () => { let sum = 0; for(let i=0; i<diceCount; i++) { let d = roll(diceSides); if (player.level >= 6 && player.archonActive && d <= 2) d = 3; sum += d; } return sum; };
    let total1 = calcRoll(); if (player.archonActive) return Math.max(total1, calcRoll()); return total1;
}

function applyFireResistance(dmg) {
    if (dmg <= 0) return 0;
    let hasRes = enemyState.current.traits.includes('fire_resistance'); let hasImm = enemyState.current.traits.includes('fire_immunity');
    let finalDmg = dmg;
    if (player.archonActive) {
        ...
        if (hasImm) { finalDmg = 0; log(`🛡️ Иммунитет к огню! Урон поглощен.`, 'system'); } 
...
export function castSpell(spell) {
    ...
    setTimeout(() => {
        ...
        if (spell.id === 'create_bonfire') { if (!isSaved) damage = getFireDamageTotal(player.level >= 5 ? 2 : 1, 8); sfx.fire(); } 
        else if (spell.id === 'thunderwave') { damage = roll(player.slotLevel + 1, 8); if (isSaved) damage = Math.floor(damage / 2); dmgType = 'грома'; sfx.thunder(); triggerShake(); } 
        else if (spell.id === 'fireball') { damage = getFireDamageTotal(8, 6); if (isSaved) damage = Math.floor(damage / 2); sfx.crit(); triggerShake(); }
...
function performSingleStrike(atkTypeStr) {
    ...
    if (player.level >= 3) fireTotal += getFireDamageTotal(isCrit ? 2 : 1, player.dmgDie) + player.dmgMod;
    else { radTotal += roll(player.dmgDie) + (isCrit ? roll(player.dmgDie) : 0) + player.dmgMod; baseDmgStr = `<span class="dmg-radiant">${radTotal} луч.</span>`; }
...
export function startEnemyTurn() {
    ...
    setTimeout(() => {
        ...
        if (isMiss && !isCrit) {
            ...
            log(`${enemyState.current.name} не пробивает Эфирную броню!`, 'enemy-turn');
        } else {
            ...
            if (ui.rebukeToggle.checked && player.spellSlots > 0 && totalDmg > 0 && enemyState.current.hp > 0) {
                ...
                log(`🌋 Вы применяете <b>Адское возмездие</b>! Враг получает <span class="dmg-fire">${rebukeDmg} огн.</span> урона.`, 'player-turn');
            }
        }
