...
function applyFireResistance(dmg) {
    ...
    if (player.archonActive) {
        if (hasImm) {
            ...
            log(`🔥 <b>Мощь Архонта:</b> Иммунитет к огню пробит (считается как сопротивление)!`, 'system');
        } else if (hasRes) {
        ...
        if (hasImm) {
            ...
            finalDmg = Math.floor(finalDmg / 2);
            log(`🛡️ Сопротивление огню! Огненный урон снижен вдвое.`, 'system');
        }
    }
    return finalDmg;
}
...
export function startGame() {
    ...
    player.reset(); gameState.stage = 1; startStage();
}
...
function startStage() {
    ...
    enemyState.generate(gameState.stage);
    renderNewEnemy();
...
function performSingleStrike(atkTypeStr) {
    ...
    if (ui.fireToggle.checked && !player.fireStrikeUsedThisTurn && player.currentGiantStrikeCharges > 0) {
        ...
        giantFireTotal = getFireDamageTotal(isCrit ? 2 : 1, 10);
    ...
    if (enemyState.current.hp <= 0 && enemyState.current.traits.includes('undead_fortitude') && !enemyState.current.usedFortitude) {
        if (Math.random() < 0.5) {
            enemyState.current.hp = 1; enemyState.current.usedFortitude = true;
            log(`💀 <b>Стойкость нежити!</b> Враг отказывается умирать и остается с 1 ХП!`, 'enemy-turn');
        }
    }
}
...
function checkCombatState() {
    ...
    if (enemyState.current.hp <= 0) {
        ...
        log(`<i>Бой окончен. Отдохните или загляните в магазин перед следующей битвой.</i>`, 'system');
        updateUI(); 
        return true;
    } else if (player.hp <= 0) {
